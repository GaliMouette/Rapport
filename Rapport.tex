...
\begin{enumerate}
    \item Explication pourquoi prototype : complexité, difficulté, choix techniques, familiarisation avec les outils de développement, \dots
    \item Élimine probl metalanage coq, concernter sur ce que je faire et pas les problèmes de coq
\end{enumerate}
\subsection{Le mini assistant de preuve}
\begin{enumerate}
    \item Présentation travail de Louise, implémentation en OCaml, déduction naturelle intuitionniste propositionnelle, fonctionnement (noyau, tactiques, vérification, construction arbre preuve), \dots
    \item Exemple preuve avant modifs ?
\end{enumerate}
...
\begin{enumerate}
    \item Démo avec $\land E$, $\land E_L$ et $\land E_R$.
    \item Ajout ne change pas la logique, juste simplifie construction avec moins d'étapes
    \item Effectivement génère moins de sous-buts
    \item Pas de problème de construction, car dérivable
    \item Expliquer dérivable : instances spécifiques de la règle $\land E$, correspond à un arbre en lui-même. Peut-être mettre dérivation en annexe ?
\end{enumerate}
\subsubsection{Implémentation}
Se référer à la rédaction précédente
\begin{enumerate}
    \item Erreur : pas respectée séparation noyau/tactiques, pas vérifié que ça n'avait pas d'impacts indésirés
    \item Retour en arrière, composition avec tactiques existantes
    ...
    \item Première étape vers la composition de tactique
\end{enumerate}
